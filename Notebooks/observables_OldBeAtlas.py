...
import glob as glob
import numpy as np
# Parche de compatibilidad para código viejo que usa np.product
...
# Configuramos warnings para ignorar específicamente las advertencias de división inválida
...
## Verifica que fracción de la grilla esta completa
##Tiene sentido 1==1 si se quiere inspeccionar cuantos modelos faltan
...
## Imprimir algo de contents[4] para todos los modelos dejar en 1==2
...
def HDUST_Lband_spectra(contents,lamb_left,lamb_right,file_pars):
    ...
    for incs in range(0,len(contents[1])):
        ...
        ylp = np.array([el*contents[4][3]*phc.Lsun.cgs*1e-4/\
                ...
                for el in ylp])
	### Solo sirve para ver el SED crudo
        if 1==2:
...
## Se debe realizar la correccion propuesta por Yanina para la linea Pf, Gamma
factor =  [0.909*spt.hydrogenlinewl(16, 6)*1e6,1000.]
### Central lambda [microns], width [km/s]
lbc_Bralpha = [spt.hydrogenlinewl(5, 4)*1e6,1000.]
lbc_Pfgamma = [spt.hydrogenlinewl(8, 5)*1e6-factor[1],1000.]
